...
| Matchmaking | `interests`, `conversations`, `messages` | Interests are profile-keyed; chat is account-keyed. Messages carry `redactedCount` so repeated attempts to pass a number across are visible without storing the number. A conversation with a `bookingId` is *about* that job, and its rules come from the booking: it opens when the advance is held and stops taking messages when the job ends. Pair uniqueness is two partial indexes, because NULLs never collide in a Postgres unique index |
...
| `/chat` | 5 | 0 | Direct messages, history, conversations, read receipts, presence. A booking's own thread is not here — it lives under `/bookings/:id/messages`, where the payment and job state that gate it are |
