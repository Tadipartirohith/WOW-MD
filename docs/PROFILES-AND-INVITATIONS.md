...
an email and a mobile number; the subject sets their own credentials on the
invitation page. That is what stops an agent being able to sign in as their own
client, and it is why there is no "create client account" endpoint any more.
...
| `POST /agents/profiles` | steward | Build a profile (email + mobile required) |
